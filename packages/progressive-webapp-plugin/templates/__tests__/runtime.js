...
describe('[progressive-webapp-plugin/templates] runtime', function() {
  ...
  it('> requestNotificationsPermission should try to subscribe', function() {
    ...
    expect(navigator.serviceWorker.pushManager.subscribe.mock.calls.length).toEqual(1);
    expect(navigator.serviceWorker.pushManager.subscribe.mock.calls[0][0]).toEqual({ userVisibleOnly: true });
  });
